...
class VQGanGumbelVAE(nn.Module):
    ...
    def decode(self, img_seq):
        b, n = img_seq.shape
        one_hot_indices = nn.functional.one_hot(img_seq, num_classes=self.num_tokens).float()
...
class GumbelQuantize(nn.Module):
    ...
    def forward(self, z, temp=None, return_logits=False):
        ...
        diff = self.kl_weight * sum(qy * log(qy * self.n_embed + 1e-10), dim=1).mean()
